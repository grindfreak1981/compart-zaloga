...
export default function Poraba() {
  ...
  const canStockOut = hasPermission(profile, PERMISSIONS.STOCK_OUT)
  ...
  const fetchMovements = async () => {
    const { data } = await supabase.from("stock_movements").select("*").eq("type", "out").order("created_at", { ascending: false }).limit(10)
    setMovements(data || [])
  }
  ...
  const handlePoraba = async () => {
    ...
    if (!material) return
    if (quantity > material.current_stock) { setError(`Ni dovolj zaloge! Na zalogi je samo ${material.current_stock} ${material.unit}.`); return }
    setError("")
    await supabase.from("stock_movements").insert([{ material_id: selectedMaterial, type: "out", quantity, note }])
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", fontFamily: "sans-serif" }}>
      <Sidebar active="/poraba" profile={profile} />

      <div style={{ flex: 1, background: "#f8fafc", padding: isMobile ? "16px" : "32px" }}>
        <div style={{ marginBottom: "24px" }}>
          <h1 style={{ fontSize: isMobile ? "22px" : "28px", fontWeight: "bold", color: "#111827", margin: 0 }}>Poraba materiala</h1>
          <p style={{ color: "#6b7280", fontSize: "14px", margin: "4px 0 0 0" }}>Zabeleži porabo materiala iz skladišča.</p>
        </div>

        {!canStockOut ? (
          <div style={{ background: "#fff5f5", border: "1px solid #fca5a5", borderRadius: "8px", padding: "24px", color: "#dc2626", fontSize: "14px" }}>
            🔒 Nimaš dovoljenja za beleženje porabe.
          </div>
        ) : (
          <>
            {success && <div style={{ background: "#f0fdf4", border: "1px solid #86efac", borderRadius: "8px", padding: "14px 18px", marginBottom: "24px", color: "#16a34a", fontWeight: "500", fontSize: "14px" }}>✅ Poraba uspešno zabeležena!</div>}
            {error && <div style={{ background: "#fff5f5", border: "1px solid #fca5a5", borderRadius: "8px", padding: "14px 18px", marginBottom: "24px", color: "#dc2626", fontWeight: "500", fontSize: "14px" }}>⚠️ {error}</div>}
                    ...
                    {movements.map((m, i) => {
                      ...
                      return (
                        <tr key={m.id} style={{ borderBottom: "1px solid #f3f4f6", background: i % 2 === 0 ? "white" : "#fafafa" }}>
                          ...
                          <td style={{ padding: "10px 14px", fontSize: "13px", fontWeight: "600", color: "#dc2626", whiteSpace: "nowrap" }}>-{m.quantity} {mat?.unit}</td>
